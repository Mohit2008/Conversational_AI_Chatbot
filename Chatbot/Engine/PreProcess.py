...
def pre_process():
    ...
    lines = open(chatBotConfig.input_file_name[0], encoding = 'utf-8', errors = 'ignore').read().split('\n')
    conversations = open(chatBotConfig.input_file_name[1], encoding = 'utf-8', errors = 'ignore').read().split('\n')
    ...
    for word, count in word2count.items():
        if count >= chatBotConfig.threshold_questions:
            questionswords2int[word] = word_number
        if count >= chatBotConfig.threshold_answers:
